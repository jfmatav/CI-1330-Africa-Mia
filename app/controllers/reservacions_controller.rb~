class ReservacionsController < ApplicationController
  ...
  def new
    ...
    @reservacion.id_tour = params[:id_tour]    
  end
  ...
  def create
    ...
    respond_to do |format|
      if @reservacion.save
        ...
        @tour = Tour.find(@reservacion.id_tour)
	@tour.cupo_disponible = @tour.cupo_disponible - 
				(@reservacion.num_cupos_nino
				+@reservacion.num_cupos_adulto)
	@tour.update(cupo_disponible)
      else
  ...
  private
    ...
    def reservacion_params
      params.require(:reservacion).permit(:id_tour, :num_cupos_adulto, :num_cupos_nino, :id_cliente, :id_estado)
      if params[:tour] then
          params.require(:tour).permit(:id_tour, :fecha, :hora)
      end
    end
end
